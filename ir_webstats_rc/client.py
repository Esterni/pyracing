...
class iRWebStats:
    ...
    # URL obtained from iracing.js
    # TODO proper logout
    def logout(self):
        self.logged = False  
        self.__req(ct.URL_IRACING_LOGOUT, cookie=self.last_cookie)
    ...
    def __get_irservice_info(self, resp):
        ...
        for i in items:
            ...
            try:
                ...
                pprint(
                    "Error ocurred. Couldn't get {}".format(i), self.verbose)
    ...
    def yearly_stats(self, custid=None):
        ...
        """
        r = self.__req(ct.URL_YEARLY_STATS, cookie=self.last_cookie)
    ...
    def cars_driven(self, custid=None):
        ...
        """
        r = self.__req(ct.URL_CARS_DRIVEN, cookie=self.last_cookie)
    ...
    def personal_best(self, custid=None, carid=0):
        ...
        """
        r = self.__req(ct.URL_PERSONAL_BEST, cookie=self.last_cookie)
    ...
    def lastrace_stats(self, custid=None):
        ...
        """
        r = self.__req(ct.URL_LASTRACE_STATS, cookie=self.last_cookie)
    ...
    def results_archive(self, custid=None, race_type=ct.RACE_TYPE_ROAD,
        event_types= ct.ALL, official=ct.ALL, license_level=ct.ALL,
        car=ct.ALL, track=ct.ALL, series=ct.ALL, season=(2016, 3, ct.ALL),
        date_range=ct.ALL, page=1, sort=ct.SORT_TIME, order=ct.ORDER_DESC):
    ...
    @logged_in
    def season_standings(self, season, carclass,
                        club=ct.ALL,
                        raceweek=ct.ALL,
                        division=ct.ALL,
                        sort=ct.SORT_POINTS,
    ...
    def current_series_images(self):
        # Series images are also available in /GetRaceGuide
    ...
    def subsession_results(self, subsession):
        ...
        """
        # TODO Variable URL_GET_SUBSESSRESULTS does not exist
        r = self.__req(
            ct.URL_GET_SUBSESSRESULTS % (subsession),
            useget=True
            )
